import React, { useState, useRef } from "react";
 ...
 343|
  344|      const steps = [t.step1, t.step2, t.step3];
  345|      const cardStyle = { background: c.card, borderRadius: 18, padding: "20px 18px", marginBottom: 16, border:
  "1.5px
     |
       ^
  346|    solid " + c.border };
  347|      const btnP = { flex: 1, padding: "14px", borderRadius: 12, border: "none", background: c.primary, color:

> Error: Command "npm run build" exited with 1

● The string "1.5px solid " got line-wrapped during copy-paste, splitting it across lines 345–346. Fix it by joining
  them back into one line in GitHub.

  Line 345 should be:
      const cardStyle = { background: c.card, borderRadius: 18, padding: "20px 18px", marginBottom: 16, border: "1.5px
  solid " + c.border };

  Delete the line break between "1.5px and solid " so it's all on one line. Save and redeploy — that should fix the
  build.
...
● I can't access your clipboard directly. The easiest way is to:

  1. Click the code block above to select it
  2. Or manually copy this line:
...
> can you make it easier to copy?

● Here it is as plain text, no formatting:

  const cardStyle = { background: c.card, borderRadius: 18, padding: "20px 18px", marginBottom: 16, border: "1.5px solid
   " + c.border };

✻ Sautéed for 2s

> like to just re-paste the entire thing
...
● Here's the full code:
  ...
  const lang = {
    en: {
      ...
      step1: "Kid's Profile", step2: "Parent Details", step3: "Performance & Showcase",
      kidNameEN: "Full Name (English)", kidNameENPh: "e.g. Somchai Jaidee",
      kidNameTH: "Full Name (Thai)", kidNameTHPh: "e.g. Somchai Jaidee",
      kidAge: "Age", kidAgePh: "e.g. 10",
      kidGolfExp: "Golf Experience (years)", kidGolfExpPh: "e.g. 3",
      kidAcadYear: "Academic Year / Grade", kidAcadYearPh: "e.g. Grade 5",
      kidSchool: "School", kidSchoolPh: "e.g. Bangkok Christian College",
      ...
      colors: "Theme", registerAnother: "+ Register Another",
      activityLabel: "Activity", parentLabel: "Parent", phoneLabel: "Phone",
      noFiles: "No files required for this activity",
      submitting: "Submitting...", networkError: "Network error. Please try again.",
    },
    th: {
      brand: "all Thailand", brandSub: "Golf Clinic",
      ...
      colors: "ธีม", registerAnother: "+ ลงทะเบียนอีกคน",
      activityLabel: "กิจกรรม", parentLabel: "ผู้ปกครอง", phoneLabel: "โทรศัพท์",
      noFiles: "ไม่จำเป็นต้องอัปโหลดไฟล์สำหรับกิจกรรมนี้",
      submitting: "กำลังส่งข้อมูล...", networkError: "เกิดข้อผิดพลาด กรุณาลองใหม่",
    },
  };
  ...
  const css = [
    ...
    "@keyframes breathe { 0%{transform:scale(1);opacity:0.85} 50%{transform:scale(1.18);opacity:1}
  100%{transform:scale(1);opacity:0.85} }",
    "@keyframes btnBreathe { 0%{box-shadow:0 0 0px rgba(245,184,0,0);transform:scale(1)} 50%{box-shadow:0 0 22px
  rgba(245,184,0,0.6);transform:scale(1.03)} 100%{box-shadow:0 0 0px rgba(245,184,0,0);transform:scale(1)} }",
    "@keyframes shimmer { 0%{opacity:0.6} 50%{opacity:1} 100%{opacity:0.6} }",
    ".fade-in { animation: fadeIn 0.4s ease both }",
    ".fade-up { animation: fadeUp 0.5s ease both }",
    ".fade-out { animation: fadeOut 0.4s ease forwards }",
    ...
    ".shimmer { animation: shimmer 2s ease-in-out infinite }",
    ".hover-card { transition: transform 0.18s ease, box-shadow 0.18s ease !important }",
    ".hover-card:hover { transform: scale(1.02) !important; box-shadow: 0 0 20px rgba(245,184,0,0.27) !important }",
    ".hover-flag { transition: transform 0.18s ease, box-shadow 0.18s ease !important }",
    ".hover-flag:hover { transform: scale(1.08) !important; box-shadow: 0 0 16px rgba(245,184,0,0.47) !important }",
    "input:focus { border-color: #F5B800 !important; outline: none; }",
    "* { box-sizing: border-box; }",
    "body { margin: 0; }",
  ].join("\n");
  ...
  function Anim({ cls = "fade-up", delay = 0, style, children }) {
    return <div className={cls} style={{ animationDelay: delay + "s", ...style }}>{children};
  }

  function DetailLine({ item, color }) {
    return (
      <div style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 7 }}>
        <span style={{ color: "#F5B800", fontSize: 14, marginTop: 1, flexShrink: 0 }}>*
        <span style={{ fontSize: 13, color, lineHeight: 1.5 }}>
          {item.text}{item.bold ? {item.bold} : null}{item.after || ""}
  ...
  function SectionDivider({ label, c }) {
    return <div style={{ fontSize: 10, color: c.primary, letterSpacing: 2, textTransform: "uppercase", fontWeight: 700,
  marginBottom: 12, marginTop: 8, paddingBottom: 6, borderBottom: "1px solid " + c.primary + "44" }}>{label};
  }
  ...
  function Input({ label, placeholder, type = "text", value, onChange, error, c }) {
   ...
   ? c.primary : c.border), fontSize: 15, background: "rgba(128,128,128,0.08)", color: c.textMain, outline: "none" }}
        />
        {error && <div className="fade-in" style={{ fontSize: 12, color: "#E55", marginTop: 5 }}>! {error}}

    );
  }

  function TwoCol({ children }) {
    return <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>{children};
  }
  ...
  borderRadius: 12, cursor: "pointer", textAlign: "center", border: "2px solid " + (sel ? c.primary : error ? "#E55" :
  c.border), background: sel ? c.primary + "18" : "rgba(128,128,128,0.08)", transition: "all 0.2s" }}>
                {opt.icon && <div style={{ fontSize: 18, marginBottom: 4 }}>{opt.icon}}
                <div style={{ fontSize: 12, fontWeight: sel ? 700 : 400, color: sel ? c.primary : c.textMain,
  lineHeight: 1.3 }}>{opt.label}
  ...
  background: value ? c.primary + "18" : "rgba(128,128,128,0.08)", transition: "all 0.25s" }}>
          <span style={{ fontSize: 13, color: value ? c.primary : error ? "#E55" : c.textSub, fontWeight: value ? 600 :
  400 }}>{value ? value.name : label}
          <input ref={ref} type="file" accept="*/*" style={{ display: "none" }} onChange={e =>
  onChange(e.target.files[0])} />
  ...
  "transparent", color: c.textSub, fontSize: 14, fontWeight: 600, cursor: "pointer", letterSpacing: 1 };
    const genderOpts = [{ value: "male", label: t.genderMale, icon: "M" }, { value: "female", label: t.genderFemale,
  icon: "F" }, { value: "other", label: t.genderOther, icon: "-" }];
    const certOpts = [{ value: "yes", label: t.certYes }, { value: "no", label: t.certNo }];
    ...
    {step === -1 && (
      <div className={splashOut ? "fade-out" : "fade-in"} style={{ minHeight: "100vh", display: "flex", flexDirection:
  "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", textAlign: "center" }}>
  ...
  marginBottom: 8 }}>{t.title}</div>
        ...
        <Anim cls="fade-up" delay={0.5} style={{ width: "100%", maxWidth: 360 }}>
          <button className="btn-breathe" style={{ ...btnP, width: "100%", fontSize: 16, padding: "18px", letterSpacing:
   4, borderRadius: 14 }} onClick={handleGetStarted}>GET STARTED</button>
        </Anim>
        <Anim cls="fade-up" delay={0.7}><div className="shimmer" style={{ fontSize: 12, color: "#555", marginTop: 24,
  letterSpacing: 1 }}>all Thailand Golf Tour</div></Anim>
      </div>
    )}

    {showLangPicker && (
      <div className="fade-in" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex",
      ...
      <div className="fade-in" style={{ background: c.card, borderBottom: "3px solid " + c.primary, padding: "16px 20px"
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
            <div style={{ display: "flex", background: "rgba(128,128,128,0.15)", borderRadius: 20, overflow: "hidden",
  ...
  l ? c.headerText : c.textSub, borderRadius: 20, letterSpacing: 1, transition: "all 0.2s" }}>{l.toUpperCase()}</button>
              ))}
            </div>
  ...
  c.border, borderRadius: 20, padding: "4px 10px", color: c.textSub, fontSize: 11, cursor: "pointer"
        ...
        {showTheme && (
          <div className="fade-up" style={{ display: "flex", gap: 8, marginTop: 12 }}>
            {THEMES.map((th, i) => (
              <button key={i} onClick={() => { setThemeIdx(i); setShowTheme(false); }} style={{ background: th.primary,
  border: i === themeIdx ? "2px solid " + (i === 0 ? "#333" : "#FFF") : "2px solid transparent", borderRadius: 20,
  padding: "5px 14px", color: th.headerText, fontSize: 12, cursor: "pointer", fontWeight: 700 }}>{language === "th" ?
  th.nameT : th.name}</button>
            ))}
          </div>
        )}
        {step > 0 && step < 4 && (
  ...
  === "#F5F5F0" ? "#DDD" : "#333"), color: i + 1 <= step ? c.headerText : c.textSub, border: i + 1 === step ? "2px solid
   " + c.primary : "2px solid transparent", transition: "all 0.3s" }}>{i + 1 < step ? "v" : i + 1}</div>
                  <span style={{ fontSize: 10, color: i + 1 <= step ? c.primary : c.textSub, letterSpacing: 0.5
  }}>{s}</span>
                </div>
                {i < 2 && <div style={{ flex: 1, height: 1.5, background: i + 1 < step ? c.primary : c.border, margin:
  "0 8px", transition: "background 0.4s" }} />}
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ padding: "20px 16px", maxWidth: 480, margin: "0 auto" }}>
        ...
        {step === 0 && (
          <div key={animKey} className="slide-in">
            <div style={{ fontSize: 13, color: c.textSub, letterSpacing: 2, textTransform: "uppercase", marginBottom: 2
  }}>* {t.selectDay} *</div>
            ...
            {[{ label: t.saturday, details: SAT_DETAILS }, { label: t.sunday, details: SUN_DETAILS }].map((item, idx) =>
   {
              ...
              return (
                <Anim key={item.label} cls="fade-up" delay={idx * 0.1}>
                  <div className="hover-card" onClick={() => toggleDay(item.label)} style={{ background: sel ? c.primary
   + "14" : c.card, borderRadius: 18, marginBottom: 14, cursor: "pointer", border: "2px solid " + (sel ? c.primary :
  c.border), overflow: "hidden", transition: "all 0.25s" }}>
  ...
  flexShrink: 0, transition: "all 0.2s" }}>G</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 18, fontWeight: 700, color: sel ? c.primary : c.textMain
  }}>{item.label}</div>
                        <div style={{ fontSize: 11, color: c.textSub, letterSpacing: 1 }}>JUNIOR ACTIVITY - all Thailand
   GOLF TOUR</div>
                      </div>
  ...
  setF("parentName", v)} error={errors.parentName} c={c} />
              <Input label={t.parentPhone} placeholder={t.parentPhonePh} type="tel" value={form.parentPhone} onChange={v
   => setF("parentPhone", v)} error={errors.parentPhone} c={c} />
        ...
        {step === 4 && (
          <div key={animKey} style={{ textAlign: "center" }}>
            <Anim cls="pop-in" delay={0.1}>
              <div style={{ width: 80, height: 80, borderRadius: "50%", background: c.primary, margin: "0 auto 20px",
  display: "flex", alignItems: "center", justifyContent: "center", fontSize: 38 }}>G</div>
            </Anim>
            <Anim cls="fade-up" delay={0.3}>
