...
function SectionDivider({ label, c }) {
  return <div style={{ fontSize: 10, color: c.primary, letterSpacing: 2, textTransform: "uppercase", fontWeight: 700, marginBottom: 12, marginTop: 8, paddingBottom: 6, borderBottom: "1px solid " + c.primary + "44" }}>{label}</div>;
}
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {options.map(opt => {
          ...
          return (
            <div key={opt.value} onClick={() => onChange(opt.value)} style={{ flex: 1, padding: "10px 6px", borderRadius: 12, cursor: "pointer", textAlign: "center", border: "2px solid " + (sel ? c.primary : error ? "#E55" : c.border), background: sel ? c.primary + "18" : "rgba(128,128,128,0.08)", transition: "all 0.2s" }}>
...
function FileUpload({ label, value, onChange, error, c, videoOnly }) {
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      <div onClick={() => ref.current.click()} style={{ border: "2px dashed " + (error ? "#E55" : value ? c.primary : c.border), borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer", background: value ? c.primary + "18" : "rgba(128,128,128,0.08)", transition: "all 0.25s" }}>
        <span style={{ fontSize: 20 }}>{value ? "✅" : videoOnly ? "🎬" : "📎"}</span>
        <span style={{ fontSize: 13, color: value ? c.primary : error ? "#E55" : c.textSub, fontWeight: value ? 600 : 400 }}>{value ? value.name : label}</span>
        <input ref={ref} type="file" accept={videoOnly ? "video/*" : "*/*"} style={{ display: "none" }} onChange={e => onChange(e.target.files[0])} />
      </div>
      {error && <div className="fade-in" style={{ fontSize: 12, color: "#E55", marginTop: 5 }}>⚠ {error}</div>}
    </div>
  );
...
export default function App() {
  ...
  const cardStyle = { background: c.card, borderRadius: 18, padding: "20px 18px", marginBottom: 16, border: "1.5px solid " + c.border };
  const btnP = { flex: 1, padding: "14px", borderRadius: 12, border: "none", background: c.primary, color: c.headerText, fontSize: 14, fontWeight: 700, cursor: "pointer", letterSpacing: 1.5, transition: "all 0.2s" };
  const btnO = { flex: 1, padding: "14px", borderRadius: 12, border: "2px solid " + c.border, background: "transparent", color: c.textSub, fontSize: 14, fontWeight: 600, cursor: "pointer", letterSpacing: 1 };
  const genderOpts = [{ value: "male", label: t.genderMale, icon: "👦" }, { value: "female", label: t.genderFemale, icon: "👧" }, { value: "other", label: t.genderOther, icon: "🤍" }];
    ...
    <div style={{ background: c.bg, minHeight: "100vh", fontFamily: "Arial,sans-serif" }}>
      ...
      {step === -1 && (
        <div className={splashOut ? "fade-out" : "fade-in"} style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", textAlign: "center" }}>
          <Anim cls="pop-in" delay={0.1}><div className="breathe" style={{ fontSize: 72, marginBottom: 24 }}>⛳</div></Anim>
          <Anim cls="fade-up" delay={0.3}>
            <div style={{ fontSize: 11, color: "#AAA", letterSpacing: 3, textTransform: "uppercase", marginBottom: 4 }}>{t.brand} <span style={{ color: c.primary }}>{t.brandSub}</span></div>
          ...
          <Anim cls="fade-up" delay={0.5} style={{ width: "100%", maxWidth: 360 }}>
            <button className="btn-breathe" style={{ ...btnP, width: "100%", fontSize: 16, padding: "18px", letterSpacing: 4, borderRadius: 14 }} onClick={handleGetStarted}>GET STARTED</button>
          </Anim>
          <Anim cls="fade-up" delay={0.7}><div className="shimmer" style={{ fontSize: 12, color: "#555", marginTop: 24, letterSpacing: 1 }}>all Thailand Golf Tour</div></Anim>
        </div>
      )}
        ...
        <div className="fade-in" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
          <div className="pop-in" style={{ background: c.card, borderRadius: 24, padding: "36px 32px", textAlign: "center", width: 300, border: "2px solid " + c.primary }}>
            ...
            <div style={{ display: "flex", gap: 20, justifyContent: "center" }}>
              {[{ code: "en", label: "English", sublabel: "อังกฤษ", flag: ukFlag }, { code: "th", label: "ภาษาไทย", sublabel: "Thai", flag: thFlag }].map(opt => {
                const sel = language === opt.code;
                return (
                  <div key={opt.code} className="hover-flag" onClick={() => selectLanguage(opt.code)} style={{ cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 10, padding: "14px 18px", borderRadius: 16, border: "2px solid " + (sel ? c.primary : "transparent"), background: sel ? c.primary + "18" : "transparent", transition: "all 0.2s" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
              <div style={{ display: "flex", background: "rgba(128,128,128,0.15)", borderRadius: 20, overflow: "hidden", border: "1px solid " + c.border }}>
                {["en", "th"].map(l => (
                  <button key={l} onClick={() => setLanguage(l)} style={{ padding: "5px 14px", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, background: language === l ? c.primary : "transparent", color: language === l ? c.headerText : c.textSub, borderRadius: 20, letterSpacing: 1, transition: "all 0.2s" }}>{l.toUpperCase()}</button>
                ))}
              </div>
              <button onClick={() => setShowTheme(!showTheme)} style={{ background: "transparent", border: "1px solid " + c.border, borderRadius: 20, padding: "4px 10px", color: c.textSub, fontSize: 11, cursor: "pointer" }}>🎨 {t.colors}</button>
            </div>
          </div>
          {showTheme && (
            <div className="fade-up" style={{ display: "flex", gap: 8, marginTop: 12 }}>
              {THEMES.map((th, i) => (
                <button key={i} onClick={() => { setThemeIdx(i); setShowTheme(false); }} style={{ background: th.primary, border: i === themeIdx ? "2px solid " + (i === 0 ? "#333" : "#FFF") : "2px solid transparent", borderRadius: 20, padding: "5px 14px", color: th.headerText, fontSize: 12, cursor: "pointer", fontWeight: 700 }}>{language === "th" ? th.nameT : th.name}</button>
              ))}
            </div>
          )}
          {step > 0 && step < 4 && (
              ...
              {steps.map((s, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", flex: i < 2 ? 1 : "none" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <div style={{ width: 24, height: 24, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, flexShrink: 0, background: i + 1 <= step ? c.primary : (c.bg === "#F5F5F0" ? "#DDD" : "#333"), color: i + 1 <= step ? c.headerText : c.textSub, border: i + 1 === step ? "2px solid " + c.primary : "2px solid transparent", transition: "all 0.3s" }}>{i + 1 < step ? "✓" : i + 1}</div>
        ...
        <div style={{ padding: "20px 16px", maxWidth: 480, margin: "0 auto" }}>
          ...
          {step === 0 && (
            <div key={animKey} className="slide-in">
              ...
              {[{ label: t.saturday, details: SAT_DETAILS }, { label: t.sunday, details: SUN_DETAILS }].map((item, idx) => {
                const sel = days.includes(item.label);
                return (
                  <Anim key={item.label} cls="fade-up" delay={idx * 0.1}>
                    <div className="hover-card" onClick={() => toggleDay(item.label)} style={{ background: sel ? c.primary + "14" : c.card, borderRadius: 18, marginBottom: 14, cursor: "pointer", border: "2px solid " + (sel ? c.primary : c.border), overflow: "hidden", transition: "all 0.25s" }}>
          ...
          {step === 1 && (
            <div key={animKey} className="slide-in">
              <div style={cardStyle}>
                ...
                <Input label={t.kidNameEN} placeholder={t.kidNameENPh} value={form.kidNameEN} onChange={v => setF("kidNameEN", v)} error={errors.kidNameEN} c={c} />
                <Input label={t.kidNameTH} placeholder={t.kidNameTHPh} value={form.kidNameTH} onChange={v => setF("kidNameTH", v)} error={errors.kidNameTH} c={c} />
                <SectionDivider label="Personal Info" c={c} />
                <TwoCol>
                  <Input label={t.kidAge} placeholder={t.kidAgePh} type="number" value={form.kidAge} onChange={v => setF("kidAge", v)} error={errors.kidAge} c={c} />
                  <Input label={t.kidGolfExp} placeholder={t.kidGolfExpPh} type="number" value={form.kidGolfExp} onChange={v => setF("kidGolfExp", v)} error={errors.kidGolfExp} c={c} />
                </TwoCol>
                <ChoiceSelect label={t.gender} value={form.kidGender} onChange={v => setF("kidGender", v)} error={errors.kidGender} c={c} options={genderOpts} />
                <SectionDivider label="Academic Info" c={c} />
                <TwoCol>
                  <Input label={t.kidAcadYear} placeholder={t.kidAcadYearPh} value={form.kidAcadYear} onChange={v => setF("kidAcadYear", v)} error={errors.kidAcadYear} c={c} />
                  <Input label={t.kidProvince} placeholder={t.kidProvincePh} value={form.kidProvince} onChange={v => setF("kidProvince", v)} error={errors.kidProvince} c={c} />
                </TwoCol>
                <Input label={t.kidSchool} placeholder={t.kidSchoolPh} value={form.kidSchool} onChange={v => setF("kidSchool", v)} error={errors.kidSchool} c={c} />
                <SectionDivider label="Certificate" c={c} />
                <ChoiceSelect label={t.kidCert} value={form.kidCert} onChange={v => setF("kidCert", v)} error={errors.kidCert} c={c} options={certOpts} />
                <SectionDivider label="Photo" c={c} />
                <FileUpload label={t.kidPhoto} value={form.kidPhoto} onChange={v => setF("kidPhoto", v)} error={errors.kidPhoto} c={c} />
              </div>
          ...
          {step === 2 && (
            <div key={animKey} className="slide-in">
              <div style={cardStyle}>
                ...
                <Input label={t.parentName} placeholder={t.parentNamePh} value={form.parentName} onChange={v => setF("parentName", v)} error={errors.parentName} c={c} />
                <Input label={t.parentPhone} placeholder={t.parentPhonePh} type="tel" value={form.parentPhone} onChange={v => setF("parentPhone", v)} error={errors.parentPhone} c={c} />
                <Input label={t.parentEmail} placeholder={t.parentEmailPh} type="email" value={form.parentEmail} onChange={v => setF("parentEmail", v)} error={errors.parentEmail} c={c} />
              </div>
              <div style={{ display: "flex", gap: 10 }}>
            ...
            <div key={animKey} className="slide-in">
              <div style={cardStyle}>
                ...
                {days.includes(t.saturday)
                  ? <>
                      <FileUpload label={t.scorecard}  value={form.scorecard}  onChange={v => setF("scorecard", v)}  error={errors.scorecard}  c={c} />
                      <FileUpload label={t.swingVideo} value={form.swingVideo} onChange={v => setF("swingVideo", v)} error={errors.swingVideo} c={c} videoOnly />
                      <FileUpload label={t.introVideo} value={form.introVideo} onChange={v => setF("introVideo", v)} error={errors.introVideo} c={c} videoOnly />
                    </>
          ...
          {step === 4 && (
            <div key={animKey} style={{ textAlign: "center" }}>
              ...
              <Anim cls="fade-up" delay={0.6}>
                <button style={{ ...btnP, width: "100%", marginTop: 8 }} onClick={resetForm}>{t.registerAnother}</button>
              </Anim>
            </div>
          )}
        </div>
